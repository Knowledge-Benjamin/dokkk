...
export async function generateDoctorLetter(): Promise<string | null> {
  const records = await getAllRecords();
  const profile = await getProfile();
  ...
  const contextText = records
    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
    .slice(0, 20)
    .map(r => `[${r.date}] [${r.type}] ${r.title}: ${r.content.slice(0, 400)}`)
    .join('\n\n');

  const systemInstruction = `
    You are a Senior Medical Scribe. Generate a formal, structured Doctor's Referral Letter for this patient that a clinician can use in an appointment.
    ...
    ## Medications (if any in records)
    ## Recent Medical History (chronological summary)
    ## Key Symptoms & Trends
    ## Suggested Discussion Points

    TONE: Professional, clinical, and concise. Max 600 words.
    Add "GENERATED BY PMI BRAIN — FOR PATIENT USE ONLY" at the end.
  `;
