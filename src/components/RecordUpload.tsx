...
import { saveRecord, getProfile } from '../lib/db';
import { MedicalRecord, RecordType, UserProfile } from '../types';
...
export default function RecordUpload() {
  ...
  const processText = async (text: string, title: string, type: RecordType, source: string) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message || t.failedProcess);
    } finally {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <header className="mb-10">
        <h2 className="text-3xl font-bold text-slate-900 mb-2">{t.ingestionPipeline}</h2>
        <p className="text-slate-500">{t.secureAdd}</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        ...
        >
          ...
          <div className="text-center">
            <h4 className="font-bold text-slate-900">{t.scanPaper}</h4>
            <p className="text-sm text-slate-500">{t.ocrPhysical}</p>
          </div>
        </button>
        ...
        >
          ...
          <div className="text-center">
            <h4 className="font-bold text-slate-900">{t.uploadDigital}</h4>
            <p className="text-sm text-slate-500">{t.onDeviceProcess}</p>
          </div>
        </button>
      </div>
      ...
      {isProcessing && (
        <div className="medical-card bg-teal-50 border-teal-200 flex items-center gap-4 mb-6">
          ...
          <div>
            <p className="font-bold text-teal-900">{t.processingActive}</p>
      ...
      {success && (
        <div className="medical-card bg-emerald-50 border-emerald-200 flex items-center gap-4 mb-6">
          ...
          <div>
            <p className="font-bold text-emerald-900">{t.vaultSuccess}</p>
            <p className="text-sm text-emerald-700">{t.vaultSuccessDesc}</p>
          </div>
      ...
      {error && (
        <div className="medical-card bg-rose-50 border-rose-200 flex items-center gap-4 mb-6">
          ...
          <div>
            <p className="font-bold text-rose-900">{t.pipelineError}</p>
      ...
      <div className="mt-12 p-8 bg-white rounded-3xl border border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">{t.entryTitle}</label>
            <input 
              type="text"
              value={manualTitle}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">{t.recordType}</label>
            <select 
              value={manualType}
              onChange={(e) => setManualType(e.target.value as RecordType)}
              className="w-full p-3 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-teal-500 outline-none transition-all"
            >
              <option value="manual_entry">{t.generalNote}</option>
              <option value="symptom_log">{t.symptomLog}</option>
              <option value="prescription">{t.prescription}</option>
              <option value="visit_note">{t.visitNote}</option>
            </select>
          </div>
        </div>

        <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">{t.content}</label>
        <textarea 
          value={manualText}
